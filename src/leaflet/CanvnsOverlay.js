/*
 * create by zhiqiangc on 2019-01-24
 * leaflet-canvas
 */
L.CanvnsOverlay = (L.Layer ? L.Layer : L.Class).extend({
  // default options
  options: {},
  ...
  getEvents() {
    ...
    function getBufferedBounds(pixels) {
      ...
      let leftcorner = self._map.containerPointToLatLng(
        L.point(bufferUpperLeft)
      )
      let rightcorner = self._map.containerPointToLatLng(
        L.point(bufferLowerRight)
      )
...
L.canvnsOverlay = function(options) {
  return new L.CanvnsOverlay(options)
}
